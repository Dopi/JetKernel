...
#else	
	if (codec_dai->dai_ops.set_mode) {
		ret = codec_dai->dai_ops.set_mode(codec_dai, substream, cmd);
		if (ret < 0)
			return ret;
	}
#endif
...
#if 1
/* 
	kcontrol functions for sktlinux  (+Jeonhk 20080804)
*/
...
EXPORT_SYMBOL_GPL(snd_soc_info_audio_play_path);
int snd_soc_put_audio_play_path(struct snd_kcontrol *kcontrol, 
	struct snd_ctl_elem_value *ucontrol)
{
	int path = ucontrol->value.integer.value[0];
	int val = ucontrol->value.integer.value[1];

	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct snd_soc_dai *codec_dai = codec->dai;
	
	printk("%s ++++,  ucontrol->value.integer.value[0] = %d\n", __FUNCTION__, path );	
	printk("%s ++++,  ucontrol->value.integer.value[1] = %d\n", __FUNCTION__, val );
	
	codec->dai->dai_ops.ioctl(codec_dai, AUDIO_IOCTL_PLAY_PATH, path,val);
	
	return 0;
}
EXPORT_SYMBOL_GPL(snd_soc_put_audio_play_path);
...
EXPORT_SYMBOL_GPL(snd_soc_get_audio_play_path);
...
EXPORT_SYMBOL_GPL(snd_soc_info_audio_phone_path);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	codec->dai->dai_ops.ioctl(codec_dai, AUDIO_IOCTL_PHONE_PATH, path,val);
	
	return 0;
}
EXPORT_SYMBOL_GPL(snd_soc_put_audio_phone_path);
...
EXPORT_SYMBOL_GPL(snd_soc_get_audio_phone_path);
...
EXPORT_SYMBOL_GPL(snd_soc_info_audio_rec_path);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	codec->dai->dai_ops.ioctl(codec_dai, AUDIO_IOCTL_REC_PATH, path,val);
	
	return 0;
}
EXPORT_SYMBOL_GPL(snd_soc_put_audio_rec_path);
...
EXPORT_SYMBOL_GPL(snd_soc_get_audio_rec_path);
...
EXPORT_SYMBOL_GPL(snd_soc_info_audio_mic_path);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	codec->dai->dai_ops.ioctl(codec_dai, AUDIO_IOCTL_MIC_PATH, path,val);
	
	return 0;
}
EXPORT_SYMBOL_GPL(snd_soc_put_audio_mic_path);
...
EXPORT_SYMBOL_GPL(snd_soc_get_audio_mic_path);
#endif
