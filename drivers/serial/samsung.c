				       ...
				       struct ktermios *old)
{
	...
	if (baud == 38400 && (port->flags & UPF_SPD_MASK) == UPF_SPD_CUST) {
		...
		ubrdiv = s3c24xx_serial_getclk(port, &clksrc, &clk, baud, &udivslot);
	...
	wr_regl(port, S3C2410_UBRDIV, ubrdiv);
	wr_regl(port, S3C2410_UDIVSLOT, udivslot_table[udivslot]);
			   ...
			   int *parity, int *bits)
{
	...
	udivslot = rd_regl(port, S3C2410_UDIVSLOT);
