...
#if 0
static unsigned int save_reg[S5P_MXR_CM_COEFF_CR - S5P_MXR_STATUS + 1];
int __s5p_vm_save_reg()
{

unsigned int i,temp_reg = 0;

for(i = 0,temp_reg = mixer_base + S5P_MXR_STATUS; temp_reg <= mixer_base + S5P_MXR_CM_COEFF_CR; temp_reg += 4,i++)
	{
	save_reg[i] = readl(temp_reg);
	}		
	
}	

int __s5p_vm_restore_reg()
{
...
s5p_tv_vmx_err __s5p_vm_set_layer_show(s5p_tv_vmx_layer layer, bool show)
{
	...
	case VM_GPR0_LAYER:
//	show = 0; //mkh test for pink color problem
		mxr_config = (show) ?
	...
	VMPRINTK("\n%s;0x%x\n\n\r",__func__, readl(mixer_base + S5P_MXR_CFG));
...
// C110
	...
	case TVOUT_OUTPUT_COMPONENT_YPBPR_INERLACED:
		temp_reg &= S5P_MXR_INTERLACE_MODE;
		temp_reg &= S5P_MXR_DST_SEL_TVOUT;
		break;
				...
				u32 dst_offs_y)   //grp
{
	...
	case VM_VIDEO_LAYER:
		...
		temp_reg |= S5P_MXR_VP_ALPHA_VALUE(alpha);
		/* temp yuv pxl limiter setting*/ //mkh
		//temp_reg &= ~(1<<17);		
		writel(temp_reg, mixer_base + S5P_MXR_VIDEO_CFG);
		break;
