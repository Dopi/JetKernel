...
s5p_tv_vp_err __s5p_vp_set_top_field_address(u32 top_y_addr, u32 top_c_addr)
{
	...
	VPPRINTK("Top field address 0x%x,0x%x\n\r", readl(vp_base + S5P_VP_TOP_Y_PTR), 
...
//printk("\n +++++++ s5p_vp_set_img_size %d,%d \n",img_width,img_height);
	if (VP_IMG_SIZE_ILLEGAL(img_width) || VP_IMG_SIZE_ILLEGAL(img_height)) {  //mkh /2 removed
		//VPPRINTK
//		printk("------0000---- image full size is not double word align = %d,%d\n\r", 
//			img_width, img_height);
		return S5P_TV_VP_ERR_BASE_ADDRESS_MUST_DOUBLE_WORD_ALIGN;
	}
				...
				bool ipc_2d)
{
	...
	//mkh:
	 ...
	 //printk("\nVP addresses %x: %x ,%x,%x\n",top_y_addr,bottom_y_addr,top_c_addr,bottom_c_addr);
	 //printk("\nVP size source %d: %d ; destination %d:%d\n",src_width,src_height,dst_width,dst_height);
