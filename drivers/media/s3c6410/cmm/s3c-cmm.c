...
static int s3c_cmm_open(struct inode *inode, struct file *file)
{
	...
	ret = LockCMMMutex();
	if (!ret){
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_open", "DD::CMM Mutex Lock Fail\n");
		return -1;
	}

	// check the number of instance 
	if((inst_no = GetInstanceNo()) < 0){
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_open", "Instance Number error-too many instance\r\n");
		UnlockCMMMutex();
		return -1;
	}
	...
	if(CodecMem == NULL){
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_open", "CodecMem application failed\n");
		UnlockCMMMutex();
		return -1;
	}
...
static int s3c_cmm_release(struct inode *inode, struct file *file)
{
	...
	ret = LockCMMMutex();
	if(!ret){
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_release", "DD::CMM Mutex Lock Fail\r\n");
		return -1;
	}
	
	CodecMem = (CODEC_MEM_CTX *)file->private_data;
	LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_release", "[%d][CMM Close] \n", CodecMem->inst_no);

	if(!CodecMem){
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_close", "CMM Invalid Input Handle\r\n");
		UnlockCMMMutex();
		return -1;
	}

	LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_close", "CodecMem->inst_no : %d\n", CodecMem->inst_no);
	...
	LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_release", "[%d]instance MergeFragmentation\n", CodecMem->inst_no);
		...
		int cmd, unsigned long arg)
{
	...
	CodecMem = (CODEC_MEM_CTX *)file->private_data;
	if (!CodecMem) {
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_ioctl", "CMM Invalid Input Handle\n");
		return -1;
	}

	ret = LockCMMMutex();
	if(!ret){
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_ioctl", "DD::CMM Mutex Lock Fail\r\n");
		return -1;
	}
	...
	switch (cmd) 
	{
		case IOCTL_CODEC_MEM_ALLOC:
			...
			LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_ioctl", "IOCTL_CODEC_MEM_GET\n");
			...
			if(node == NULL){
				LOG_MSG_CMM(LOG_WARNING, "s3c_cmm_ioctl", "GetCodecVirAddr(%d)\r\n", CodecMem->inst_no);
				result = -1;
				break;
			}
		...
		case IOCTL_CODEC_MEM_FREE:
			...
			LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_ioctl", "IOCTL_CODEC_MEM_FREE\n");
			...
			if(node  == AllocMemTail){
				LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_ioctl", "invalid virtual address(0x%x)\r\n", codec_mem_free_arg.u_addr);
				result = -1;
				break;
			}
		...
		case IOCTL_CODEC_CACHE_FLUSH:
			...
			LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_ioctl", "IOCTL_CODEC_CACHE_FLUSH\n");
		...
		case IOCTL_CODEC_CACHE_INVALIDATE:
			...
			LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_ioctl", "IOCTL_CODEC_CACHE_INVALIDATE\n");
		...
		case IOCTL_CODEC_CACHE_CLEAN:
			...
			LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_ioctl", "IOCTL_CODEC_CACHE_CLEAN\n");
		...
		default : 
			LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_ioctl", "DD::CMM Invalid ioctl : 0x%X\r\n", cmd);
	}
...
int s3c_cmm_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_mmap", "vma->vm_end - vma->vm_start = %d\n", offset);
	...
	if( remap_pfn_range(vma, vma->vm_start, pageFrameNo, size,	\
				vma->vm_page_prot) ) {
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_mmap", "cmm remap error");
		return -EAGAIN;
	}
...
static int __init s3c_cmm_init(void)
{
	...
	if (h_Mutex == NULL) 
	{
		LOG_MSG_CMM(LOG_ERROR, "s3c_cmm_init", "DD::CMM Mutex Initialize error\r\n");
		return FALSE;
	}
...
static void __exit s3c_cmm_exit(void)
{
	...
	LOG_MSG_CMM(LOG_TRACE, "s3c_cmm_exit", "CMM_Deinit\n");
...
static void InsertNodeToAllocList(ALLOC_MEM_T *node, int inst_no)
{
	LOG_MSG_CMM(LOG_TRACE, "InsertNodeToAllocList", "[%d]instance (cached_p_addr : 0x%08x uncached_p_addr : 0x%08x size:%ld cacheflag : %d)\n",	\
	...
	AllocMemHead = node;
	LOG_MSG_CMM(LOG_TRACE, "InsertNodeToAllocList", "Finished InsertNodeToAllocList\n");
}

// insert node ahead of FreeMemHead
static void InsertNodeToFreeList(FREE_MEM_T *node,  int inst_no)
{
	LOG_MSG_CMM(LOG_TRACE, "InsertNodeToFreeList", "[%d]instance(startAddr : 0x%08x size:%ld  cached flag : %d)\n",	\
...
static void DeleteNodeFromAllocList(ALLOC_MEM_T *node, int inst_no)
{
	LOG_MSG_CMM(LOG_TRACE, "DeleteNodeFromAllocList", "[%d]instance (uncached_p_addr : 0x%08x cached_p_addr : 0x%08x size:%ld cacheflag : %d)\n",	\
		inst_no, node->uncached_p_addr, node->cached_p_addr, node->size, node->cacheFlag);
	
	if(node == AllocMemTail){
		LOG_MSG_CMM(LOG_TRACE, "DeleteNodeFromAllocList", "InValid node\n");
		return;
	}
...
static void DeleteNodeFromFreeList( FREE_MEM_T *node, int inst_no)
{
	LOG_MSG_CMM(LOG_TRACE, "DeleteNodeFromFreeList", "[%d]DeleteNodeFromFreeList(startAddr : 0x%08x size:%ld)\n", inst_no, node->startAddr, node->size);
	if(node == FreeMemTail){
		LOG_MSG_CMM(LOG_ERROR, "DeleteNodeFromFreeList", "InValid node\n");
		return;
	}
	...
	while(node1 != FreeMemTail){
		...
		while(node2 != FreeMemTail){
			if( (node1->startAddr + node1->size == node2->startAddr) && (node1->cacheFlag == node2->cacheFlag) ){
				...
				LOG_MSG_CMM(LOG_TRACE, "MergeFragmentation", "find merge area !! ( node1->startAddr + node1->size == node2->startAddr)\n");
				DeleteNodeFromFreeList(node2, inst_no);
				break;
			}
			else if( (node1->startAddr == node2->startAddr + node2->size) && (node1->cacheFlag == node2->cacheFlag) ){
				LOG_MSG_CMM(LOG_TRACE, "MergeFragmentation", "find merge area !! ( node1->startAddr == node2->startAddr + node2->size)\n");
...
static unsigned int GetMemArea(int allocSize, int inst_no, char cache_flag)
{
	...
	LOG_MSG_CMM(LOG_TRACE, "GetMemArea", "request Size : %ld\n", allocSize);
	
	if(FreeMemHead == FreeMemTail){
		LOG_MSG_CMM(LOG_ERROR, "GetMemArea", "all memory is gone\n");
		return(allocAddr);
	}
	...
	if(match_node != NULL) {
		LOG_MSG_CMM(LOG_TRACE, "GetMemArea", "match : startAddr(0x%08x) size(%ld) cache flag(%d)\n", 	\
	...
	{
		LOG_MSG_CMM(LOG_ERROR, "GetMemArea", "there is no suitable chunk\n");
		return 0;
	}
...
static ALLOC_MEM_T * GetCodecVirAddr(int inst_no, CODEC_MEM_ALLOC_ARG *in_param)
{
	...
	if(!p_startAddr){
		LOG_MSG_CMM(LOG_TRACE, "GetCodecVirAddr", "There is no more memory\n\r");
		in_param->out_addr = -1;
		return NULL;
	}
	...
	if(in_param->cacheFlag) {
		...
		if (p_allocMem->v_addr == NULL) {
			LOG_MSG_CMM(LOG_ERROR, "GetCodecVirAddr", "Mapping Failed [PA:0x%08x]\n\r", p_allocMem->cached_p_addr);
			return NULL;
		}
	}
	else {
	...
	LOG_MSG_CMM(LOG_TRACE, "GetCodecVirAddr", "u_addr : 0x%x v_addr : 0x%x cached_p_addr : 0x%x, uncached_p_addr : 0x%x\n", 	\
...
static void PrintList()
{
	...
	for(node1 = AllocMemHead; node1 != AllocMemTail; node1 = node1->next){
		...
		LOG_MSG_CMM(LOG_TRACE, "PrintList", "[AllocList][%d] inst_no : %d p_addr : 0x%08x v_addr:0x%08x size:%ld cacheflag : %d\n", 
	...
	for(node2 = FreeMemHead; node2 != FreeMemTail; node2 = node2->next){
			LOG_MSG_CMM(LOG_TRACE, "PrintList", "[FreeList][%d] startAddr : 0x%08x size:%ld\n", count++, node2->startAddr , node2->size);
