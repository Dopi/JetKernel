...
#if defined(CONFIG_MACH_INFOBOWLQ) && defined(USES_PINCH_DIST_MT)
	if(do_update || first) {
		...
		const char *buf, size_t size)
{
	...
	if(strncmp(buf, "UPDATE", 6) == 0 || strncmp(buf, "update", 6) == 0) {
		if(melfas_ts->fw_ver>0x03 && melfas_ts->fw_ver != 0x13) { //sdk_de13
...
#if defined(CONFIG_MACH_INFOBOWLQ)
      if(!((keycode == TOUCH_HOME || keycode == TOUCH_BACK) && get_sending_oj_event()))
#else
...
#endif    
    {
	 ...
	 static int do_report2 = false; // report a second touch point
            ...
            case 0x2: // Dual-point Touch
				...
				if(likely(melfas_ts->mt_support==true)) {
					if(gesture_code) {
						...
						melfas_ts->info.z = z;
						x2 = x;
						width_multiple = width * 2;
						if(unlikely(width_multiple > melfas_ts->info.max_y))
							width_multiple = melfas_ts->info.max_y;
						if(y+width_multiple > melfas_ts->info.max_y)
							y2 = y - (width_multiple);
						else
							y2 = y + (width_multiple);
				...
				#endif
                printk(KERN_DEBUG "[TSP] Dual-point Touch!\n");
                break;
            case 0x3: // Palm Touch
        ...
        if(do_report) {
			...
			#ifdef USES_PINCH_DIST_MT
			if(likely(melfas_ts->mt_support == true)) {
				printk("[ABS_MT] x :%4d, y :%4d, z :%4d, finger:%4d, width:%4d\n", x, y, z, finger, width); 
				...
				if(do_report2) {
					printk("[ABS_MT] x2:%4d, y2:%4d, z2:%4d, finger:%4d, width:%4d\n", x2, y2, z2, finger, width); 
