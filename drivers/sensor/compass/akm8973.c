...
/****************************** JetDroid *****************************************/
#define AKM8973_SADDR 		0x1c 	// [0011100]
...
static unsigned short probe_addr[] = { 0, AKM8973_SADDR, I2C_CLIENT_END };
#else
static unsigned short probe_addr[] = { 5/*0*/, AKM8973_SADDR, I2C_CLIENT_END };
#endif
...
static irqreturn_t akm8973_interrupt(int irq, void *dev_id)
{
	...
	disable_irq_nosync(IRQ_COMPASS_INT);
	schedule_work(&data->work);
	return IRQ_HANDLED;
}

static void ak8973_init_hw(void)
{
#if 1	
...
int akm8973_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*if (pdata == NULL) { //No need to have platform data structure
	...
	err = request_irq(IRQ_COMPASS_INT, akm8973_interrupt, IRQF_TRIGGER_HIGH,
			  "akm8973", akm);
	disable_irq(IRQ_COMPASS_INT);
	...
	/*akm->input_dev = NULL; //publish_kxsd9();
	...
	//akm->input_dev->name = "kionix-kxsd9";
			  ...
			  struct i2c_board_info *info)
{
	...
	strlcpy(info->type, AKM8973_I2C_NAME, I2C_NAME_SIZE);
...
/********* Auto detection Added for JetDroid **************/
