...
/* external functions. - power-domain.c */
extern void s3c_init_domain_power(void);
extern void s3c_set_normal_cfg(unsigned int config, unsigned int flag,
		unsigned int deviceID);
extern int s3c_wait_blk_pwr_ready(unsigned int config);
extern unsigned int get_domain_off_stat(void);
extern int domain_off_check(unsigned int config);
#endif	/* __POWER_DOMAIN_H */
