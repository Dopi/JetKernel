...
static struct s3c_uartcfg jet_uartcfgs[] __initdata = {
	[0] = {	/* Phone */
		.hwport	     = 0,
		.flags	     = 0,
		.ucon	     = UCON,
...
#if (CONFIG_JET_REV >= CONFIG_JET_REV00)
...
struct map_desc jet_iodesc[] __initdata = {
};
...
static void __init jet_map_io(void)
{
	...
	s3c_init_uarts(jet_uartcfgs, ARRAY_SIZE(jet_uartcfgs));
	s3c64xx_allocate_memory_regions();
}

static void jet_set_qos(void) 
{     
	u32 reg;     							 /* AXI sfr */     
...
static void jet_pm_power_off(void)
{
...
static int jet_notifier_call(struct notifier_block *this, unsigned long code, void *_cmd)
{
	int	mode = REBOOT_MODE_NONE;
...
static struct notifier_block jet_reboot_notifier = {
	.notifier_call = jet_notifier_call,
};

static void jet_switch_init(void)
{
...
static void __init jet_machine_init(void)
{
	...
	pm_power_off = jet_pm_power_off;

	register_reboot_notifier(&jet_reboot_notifier);
...
static int jet_sleep_gpio_table[][6] = {
	/** ALIVE PART **/
	/* GPK */
	{ GPIO_CAM_EN, GPIO_CAM_EN_AF, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE, 0, 0 },
	/* GPL */
...
{	
	...
	check_pmic();
	s3c_config_gpio_table(ARRAY_SIZE(jet_sleep_gpio_table),
			jet_sleep_gpio_table);
